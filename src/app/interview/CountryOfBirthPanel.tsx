...
import { QuestionPanelCard } from "./interview-panel-cards";
import {
  COMMON_COUNTRIES,
  getCountryByCode,
  searchCountries,
...
export function CountryOfBirthPanel({ selectedCode, onSelect }: Props) {
  ...
  const [addedFromSearch, setAddedFromSearch] = useState<Country[]>([]);
  ...
  const displayCountries = [
    ...COMMON_COUNTRIES,
    ...addedFromSearch.filter((c) => !commonCountryCodes.has(c.code)),
  ];
  ...
  function handleSelect(country: Country) {
    ...
    if (!commonCountryCodes.has(country.code)) {
      setAddedFromSearch((prev) =>
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-[#F5F6F8] px-8 py-8">
      <p className="text-xs font-semibold uppercase tracking-wider text-[var(--clinical-on-surface-variant)]">
        Chagas Check · Mandatory nurse question
      </p>
      ...
      <div className="mt-6">
        <QuestionPanelCard
          title="Select country of birth"
          hint="Used to identify Chagas disease risk. Search by name or pick from the list below."
        >
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[var(--clinical-outline)] bg-[var(--clinical-surface)] py-2.5 pl-9 pr-4 text-sm text-[var(--clinical-on-surface)] outline-none focus:border-[var(--clinical-on-surface)] focus:ring-1 focus:ring-[var(--clinical-on-surface)] placeholder:text-[var(--clinical-on-surface-variant)]"
            />
            {query && (
              <button
                type="button"
          ...
          {searchResults.length > 0 ? (
            ...
            <p className="mt-4 text-sm text-[var(--clinical-on-surface-variant)]">
